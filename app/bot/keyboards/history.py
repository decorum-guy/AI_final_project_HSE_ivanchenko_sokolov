...
def history_list(items: list[DigestHistory], page: int, total: int, per_page: int = 5) -> InlineKeyboardMarkup:
    ...
    if pages > 1:
        ...
        kb.button(text="←", callback_data=f"history:{prev_page}")
        kb.button(text="→", callback_data=f"history:{next_page}")
